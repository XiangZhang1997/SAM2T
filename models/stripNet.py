...
from smt import Mlp, initAttention, initBlock, Attention, Block, OverlapPatchEmbed, Head, Bottleneck
...
class SAM2T(nn.Module):
    ...
    def forward(self, x):
        ...
        return output
# run for drive
class SAM2T_DRIVE_s(nn.Module):
    def __init__(self, img_ch=1, output_ch=1, im_size=48, embed_dims=[32, 64, 128, 256, 512],
                 ca_num_heads=[4, 4, -1], sa_num_heads=[-1, 8, 16], mlp_ratios=[2, 2, 1],
                 qkv_bias=False, qk_scale=None, use_layerscale=False, layerscale_value=1e-4, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6),
                 Endepths=[2, 8, 1], ca_attentions=[0, 1, 0], num_stages=4, head_conv=3, expand_ratio=2, **kwargs):
        super(SAM2T_DRIVE_s, self).__init__()
        ...
        # stage0
        self.patch_embed0 = OverlapPatchEmbed(patch_size=3, stride=1, in_chans=embed_dims[0], embed_dim=embed_dims[0]) # x: (B, H*W, C)---64,2304,32 不降采样+拉平
        self.block0 = nn.ModuleList([initBlock(
            dim=embed_dims[0], ca_num_heads=ca_num_heads[0], sa_num_heads=sa_num_heads[0], mlp_ratio=mlp_ratios[0], qkv_bias=qkv_bias, qk_scale=qk_scale,
            use_layerscale=use_layerscale,
            layerscale_value=layerscale_value,
        ...
        # stage1
        self.patch_embed1 = OverlapPatchEmbed(patch_size=3, stride=2, in_chans=embed_dims[0], embed_dim=embed_dims[1]) # x: (B, H*W, C)---64,2304,32 降采样+拉平
        self.block1 = nn.ModuleList([Block(
            dim=embed_dims[1], ca_num_heads=ca_num_heads[1], sa_num_heads=sa_num_heads[1], mlp_ratio=mlp_ratios[1], qkv_bias=qkv_bias, qk_scale=qk_scale,
            use_layerscale=use_layerscale,
            layerscale_value=layerscale_value,
...
class miniunet(nn.Module):
    def __init__(self, img_ch=1, output_ch=1, im_size=48, embed_dims=[32, 64, 128, 256, 512],
        ...
        # En
        self.patch_embed1 = OverlapPatchEmbed(patch_size=3, stride=1, in_chans=img_ch, embed_dim=embed_dims[0]) 
        self.En1 = double_conv(ch_in=embed_dims[0], ch_out=embed_dims[0])

        self.patch_embed2 = OverlapPatchEmbed(patch_size=3, stride=2, in_chans=embed_dims[0], embed_dim=embed_dims[1])  
        self.En2 = double_conv(ch_in=embed_dims[1], ch_out=embed_dims[1])
    ...
    def forward(self, x):
        B = x.shape[0]
        e1, H, W = self.patch_embed1(x)
        C = e1.shape[2]
        e1 = e1.contiguous().view(B, H, W, C).permute(0, 3, 1, 2)
        e1 = self.En1(e1)

        e2, H, W = self.patch_embed2(e1)
        C = e2.shape[2]
        e2 = e2.contiguous().view(B, H, W, C).permute(0, 3, 1, 2)
        e2 = self.En2(e2)
        ...
        for blk in self.block3:
            e3 = blk(e3, H, W)
        C = e3.shape[2]
        e3 = e3.contiguous().view(B, H, W, C).permute(0, 3, 1, 2)
        ...
        return output
class miniunet_dc(nn.Module):
    def __init__(self, img_ch=1, output_ch=1, im_size=48, embed_dims=[32, 64, 128, 256, 512],
                 ca_num_heads=[4, 4, -1], sa_num_heads=[-1, 8, 16], mlp_ratios=[2, 2, 1],
                 qkv_bias=False, qk_scale=None, use_layerscale=False, layerscale_value=1e-4, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6),
                 Endepths=[2, 8, 1], ca_attentions=[0, 1, 0], num_stages=4, head_conv=3, expand_ratio=2, **kwargs):
        super(miniunet_dc, self).__init__()
        self.dc0 = DC(c_i=img_ch,c_o=embed_dims[0])
        # self.mp = nn.MaxPool2d(2,2)
        # En
        self.patch_embed1 = OverlapPatchEmbed(patch_size=3, stride=1, in_chans=embed_dims[0], embed_dim=embed_dims[0]) # x: (B, H*W, C)---64,2304,32 不降采样+拉平
        self.En1 = double_conv(ch_in=embed_dims[0], ch_out=embed_dims[0])

        self.patch_embed2 = OverlapPatchEmbed(patch_size=3, stride=2, in_chans=embed_dims[0], embed_dim=embed_dims[1]) # x: (B, H*W, C)---64,2304,32 不降采样+拉平
        self.En2 = double_conv(ch_in=embed_dims[1], ch_out=embed_dims[1])

        self.patch_embed3 = OverlapPatchEmbed(patch_size=3, stride=2, in_chans=embed_dims[1], embed_dim=embed_dims[2]) # x: (B, H*W, C)---64,2304,32 不降采样+拉平
        self.block3 = nn.ModuleList([Block(
            dim=embed_dims[2], ca_num_heads=ca_num_heads[2], sa_num_heads=sa_num_heads[2], mlp_ratio=mlp_ratios[2], qkv_bias=qkv_bias, qk_scale=qk_scale,
            use_layerscale=use_layerscale,
            layerscale_value=layerscale_value,
    ...
    def forward(self, x):
        ...
        return output
class miniunet_cnnsa(nn.Module):
    def __init__(self, img_ch=1, output_ch=1, im_size=48, embed_dims=[32, 64, 128, 256, 512],
                 ca_num_heads=[4, 4, -1], sa_num_heads=[-1, 8, 16], mlp_ratios=[2, 2, 1],
                 qkv_bias=False, qk_scale=None, use_layerscale=False, layerscale_value=1e-4, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6),
                 Endepths=[2, 8, 1], ca_attentions=[0, 1, 0], num_stages=4, head_conv=3, expand_ratio=2, **kwargs):
        super(miniunet_cnnsa, self).__init__()
        ...
        # stage0
        self.patch_embed1 = OverlapPatchEmbed(patch_size=3, stride=1, in_chans=embed_dims[0], embed_dim=embed_dims[0]) # x: (B, H*W, C)---64,2304,32 不降采样+拉平
        self.block1 = nn.ModuleList([initBlock(
            dim=embed_dims[0], ca_num_heads=ca_num_heads[0], sa_num_heads=sa_num_heads[0], mlp_ratio=mlp_ratios[0], qkv_bias=qkv_bias, qk_scale=qk_scale,
            use_layerscale=use_layerscale,
            layerscale_value=layerscale_value,
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=0.1, norm_layer=norm_layer,
            ca_attention=0 if j%2!=0 else ca_attentions[0], expand_ratio=expand_ratio)  
            for j in range(Endepths[0])]) # ca_attention=0 means CNN_SA
    ...
    def forward(self, x):
        ...
        return output
class miniunet_transen(nn.Module):
    def __init__(self, img_ch=1, output_ch=1, im_size=48, embed_dims=[32, 64, 128, 256, 512],
                 ca_num_heads=[4, 4, -1], sa_num_heads=[-1, 8, 16], mlp_ratios=[2, 2, 1],
                 qkv_bias=False, qk_scale=None, use_layerscale=False, layerscale_value=1e-4, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6),
                 Endepths=[2, 8, 1], ca_attentions=[0, 1, 0], num_stages=4, head_conv=3, expand_ratio=2, **kwargs):
        super(miniunet_transen, self).__init__()
        ...
        # stage1
        self.patch_embed2 = OverlapPatchEmbed(patch_size=3, stride=2, in_chans=embed_dims[0], embed_dim=embed_dims[1]) # x: (B, H*W, C)---64,2304,32 降采样+拉平
        self.block2 = nn.ModuleList([Block(
            dim=embed_dims[1], ca_num_heads=ca_num_heads[1], sa_num_heads=sa_num_heads[1], mlp_ratio=mlp_ratios[1], qkv_bias=qkv_bias, qk_scale=qk_scale,
            use_layerscale=use_layerscale,
            layerscale_value=layerscale_value,
        ...
        # top
        self.patch_embed3 = OverlapPatchEmbed(patch_size=3, stride=2, in_chans=embed_dims[1], embed_dim=embed_dims[2]) # x: (B, H*W, C)---64,2304,32 降采样+拉平
        self.block3 = nn.ModuleList([Block(
            dim=embed_dims[2], ca_num_heads=ca_num_heads[2], sa_num_heads=sa_num_heads[2], mlp_ratio=mlp_ratios[2], qkv_bias=qkv_bias, qk_scale=qk_scale,
            use_layerscale=use_layerscale,
            layerscale_value=layerscale_value,
    ...
    def forward(self, x):
        ...
        return output
class miniunet_skipatt(nn.Module):
    def __init__(self, img_ch=1, output_ch=1, im_size=48, embed_dims=[32, 64, 128, 256, 512],
                 ca_num_heads=[4, 4, -1], sa_num_heads=[-1, 8, 16], mlp_ratios=[2, 2, 1],
                 qkv_bias=False, qk_scale=None, use_layerscale=False, layerscale_value=1e-4, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6),
                 Endepths=[2, 8, 1], ca_attentions=[0, 1, 0], num_stages=4, head_conv=3, expand_ratio=2, **kwargs):
        super(miniunet_skipatt, self).__init__()
    ...
    def forward(self, x):
        ...
        skip2 = self.Att2(self.ds2(sk1), sk2)
        skip1 = self.Att1(sk1, self.us2(sk2))
        ...
        return output
class miniunet_cnnsa_transen(nn.Module):
    def __init__(self, img_ch=1, output_ch=1, im_size=48, embed_dims=[32, 64, 128, 256, 512],
                 ca_num_heads=[4, 4, -1], sa_num_heads=[-1, 8, 16], mlp_ratios=[2, 2, 1],
                 qkv_bias=False, qk_scale=None, use_layerscale=False, layerscale_value=1e-4, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6),
                 Endepths=[2, 8, 1], ca_attentions=[0, 1, 0], num_stages=4, head_conv=3, expand_ratio=2, **kwargs):
        super(miniunet_cnnsa_transen, self).__init__()
        ...
        # top
        self.patch_embed2 = OverlapPatchEmbed(patch_size=3, stride=2, in_chans=embed_dims[1], embed_dim=embed_dims[2]) # x: (B, H*W, C)---64,2304,32 降采样+拉平
        self.block2 = nn.ModuleList([Block(
            dim=embed_dims[2], ca_num_heads=ca_num_heads[2], sa_num_heads=sa_num_heads[2], mlp_ratio=mlp_ratios[2], qkv_bias=qkv_bias, qk_scale=qk_scale,
            use_layerscale=use_layerscale,
            layerscale_value=layerscale_value,
    ...
    def forward(self, x):
        ...
        return output
class miniunet_cnnsa_skipatt(nn.Module):
    def __init__(self, img_ch=1, output_ch=1, im_size=48, embed_dims=[32, 64, 128, 256, 512],
                 ca_num_heads=[4, 4, -1], sa_num_heads=[-1, 8, 16], mlp_ratios=[2, 2, 1],
                 qkv_bias=False, qk_scale=None, use_layerscale=False, layerscale_value=1e-4, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6),
                 Endepths=[2, 8, 1], ca_attentions=[0, 1, 0], num_stages=4, head_conv=3, expand_ratio=2, **kwargs):
        super(miniunet_cnnsa_skipatt, self).__init__()
    ...
    def forward(self, x):
        ...
        return output
class miniunet_transen_skipatt(nn.Module):
    def __init__(self, img_ch=1, output_ch=1, im_size=48, embed_dims=[32, 64, 128, 256, 512],
                 ca_num_heads=[4, 4, -1], sa_num_heads=[-1, 8, 16], mlp_ratios=[2, 2, 1],
                 qkv_bias=False, qk_scale=None, use_layerscale=False, layerscale_value=1e-4, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6),
                 Endepths=[2, 8, 1], ca_attentions=[0, 1, 0], num_stages=4, head_conv=3, expand_ratio=2, **kwargs):
        super(miniunet_transen_skipatt, self).__init__()
